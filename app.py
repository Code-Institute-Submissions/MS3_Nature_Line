...
# Local farmers search and display results
...
def display_profile(profile):
    ...
    username=username, products_sold=products_sold)
...
def profile(username):
    ...
    if session["user"]:
        return render_template("profile.html", username=username,
                             curr_user=curr_user, products_sold=products_sold)
...
def edit_product(product_id):
    if request.method == "POST":
        ...
        mongo.db.products_sold.update({"_id": ObjectId(product_id)},
                                    product_update)
        flash("Product successfully updated!")
        return redirect(url_for("profile", username=session["user"]))
    product_sold = mongo.db.products_sold.find_one(
        {"_id": ObjectId(product_id)})
    product_cat = mongo.db.product_categories.find().sort("category", 1)
    return render_template("edit_product.html",
    product_sold=product_sold, product_cat=product_cat)
...
def update_info(username):
    ...
    return render_template("update_info.html",
                        username=username, curr_user=curr_user)
